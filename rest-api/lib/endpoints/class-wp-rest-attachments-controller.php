...
class WP_REST_Attachments_Controller extends WP_REST_Posts_Controller {
	...
	public function base64_image ($src){

		$path = preg_replace('#^https?://localhost:8000/#', '', $src);
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		$path = preg_replace('#^https?://localhost:8000/#', '', wp_get_attachment_url( $post->ID ));
		...
		$trimmed_image =  'data:' . $size['mime'] . ';base64,' . $base64 ;
		...
		if ( empty( $data['media_details'] ) ) {
			...
			foreach ( $data['media_details']['sizes'] as $size => &$size_data ) {
				...
				$size_data['source_url'] = $trimmed_image;
			}
